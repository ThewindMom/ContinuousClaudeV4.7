...
function parseTranscript(transcriptPath) {
  ...
  for (const line of content.split('\n')) {
    ...
    if (toolName) {
      ...
      if (toolName.toLowerCase() === 'execute' && (entry.tool_input || {}).command) {
...
function main() {
  ...
  if (trigger === 'auto' && transcriptPath && existsSync(transcriptPath)) {
    ...
    message = '[PreCompact:auto] No transcript available. Consider manual handoff.';
  } else {
    message = '[PreCompact] Consider manual handoff before compacting.';
  }
