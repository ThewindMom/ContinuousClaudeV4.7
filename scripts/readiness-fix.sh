...
- [ ] No breaking changes' "PR template"
  ...
  # tldr-powered criteria — not auto-fixable
  dead_code|clones|complexity|tech_debt|security_scan)
    echo -e "${BOLD}$crit${NC}"
    echo -e "  ${YELLOW}SKIP${NC} Requires code changes — use \`tldr $crit\` to identify issues"
    SKIPPED=$((SKIPPED + 1))
    ;;
